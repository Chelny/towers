...
import {
  ...
  SetPasswordPayload,
  setPasswordSchema,
} from "@/app/(protected)/account/profile/change-password.schema"
...
export function ChangePasswordForm({ session }: ChangePasswordFormProps): ReactNode {
    ...
    for (const error of errors) {
      switch (error.path.replace("/", "")) {
        ...
        case "confirmPassword":
          ...
          break
        default:
          console.error(`Set Password Action: Unknown error at ${error.path}`)
          break
      }
    }
    ...
    for (const error of errors) {
      switch (error.path.replace("/", "")) {
        ...
        case "confirmNewPassword":
          ...
          break
        default:
          console.error(`Change Password Action: Unknown error at ${error.path}`)
          break
      }
    }
  ...
  return (
    <>
      <h3 className="text-lg font-semibold mb-4">Change/Set Password</h3>
      <form className="w-full" noValidate onSubmit={handleChangeSetPassword}>
        ...
        <>
          ...
          <Input
            ...
            label={isUserPasswordSet ? "New Password" : "Password"}
            autoComplete="off"
            required
          ...
          <Input
            ...
            label={isUserPasswordSet ? "Confirm New Password" : "Confirm Password"}
            autoComplete="off"
            required
            dataTestId={
        ...
        <Button type="submit" className="w-full" disabled={isLoading}>
          {isUserPasswordSet ? "Change Password" : "Set Password"}
        </Button>
      </form>
    </>
  )
