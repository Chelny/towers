...
export function LinkedSocialAccountsForm(): ReactNode {
  ...
  const errorMessages: Record<string, string> = {
    "email_doesn't_match": "The email you provided does not match the one associated with your account.",
  }
      ...
      await authClient.linkSocial(
        ...
        },
        {
          ...
          onSuccess: (ctx) => {
            ...
            setFormState({
              success: true,
              message: "The account has been linked!",
            })
  ...
  return (
    <>
      <h3 className="text-lg font-semibold mb-4">Linked Social Accounts</h3>
      {formState?.message && (
        ...
        {AUTH_PROVIDERS.map(({ name, label, icon }: AuthProviderDetails) => (
          <li key={name} className="flex items-center justify-between gap-2">
            ...
            <Button
              ...
              aria-label={isProviderLinked(name) ? `Unlink ${label}` : `Link ${label}`}
              onClick={() => handleLinkUnlink(name)}
            >
              {isProviderLinked(name) ? "Unlink" : "Link"}
            </Button>
          </li>
        ))}
      </ul>
