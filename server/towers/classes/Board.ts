...
} from "@/server/towers/classes/TowersPieceBlock"
...
export class Board {
  ...
  public async convertSurroundingBlocksToPowerBlocks(powerPiece: Piece): Promise<void> {
    ...
    await this.breakBlocks(blocksToRemove, false)
  }
  ...
  private getHoosBlocksToRemove(shouldTagRemovedByOrigin: boolean): BlockToRemove[] {
    return this.hoos.flatMap((hoo: Hoo) =>
      hoo.positions.map((position: PieceBlockPosition) => ({
        ...position,
  ...
  private getMatchingBlocksToRemove(
    ...
    return matchingBlockColors.map((position: PieceBlockPosition) => ({
      ...position,
  ...
  private removeBlocks(blockPositions: PieceBlockPosition[]): void {
    ...
    for (const { row, col } of blockPositions) {
      if (this.isWithinBoardBounds(row, col)) {
        ...
        if (isPowerBarItem(this.grid[row][col])) {
          this.addBlockToPowerBar(this.grid[row][col])
        }

        // Update block's power type and power level
        if (isTowersPieceBlock(this.grid[row][col])) {
          const blockLetter: PieceBlockLetter = this.grid[row][col].letter
