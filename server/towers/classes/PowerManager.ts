...
import { Board, BoardBlock, BoardGrid, BoardGridRow } from "@/server/towers/classes/Board"
import { MedusaPieceBlock } from "@/server/towers/classes/MedusaPieceBlock"
import { PieceBlockPosition, TowersBlockLetter, TowersBlockPowerLevel } from "@/server/towers/classes/PieceBlock"
import { PowerBar, PowerBarItem, PowerBarItemPlainObject } from "@/server/towers/classes/PowerBar"
import { SpecialDiamond, SpecialDiamondPlainObject } from "@/server/towers/classes/SpecialDiamond"
...
export class PowerManager {
  ...
  constructor(user: User, tableId: string, users: User[], tableSeatManager: TableSeatManager) {
    this.user = user
    this.tableId = tableId
    this.users = users
    this.tableSeatManager = tableSeatManager
    this.userSeat = this.tableSeatManager.getTableSeat(user)
    this.registerSocketListeners()
  }

  private get board(): Board | null | undefined {
    return this.userSeat?.board
  }

  private get grid(): BoardGrid | undefined {
    return this.userSeat?.board?.grid
  }
  ...
  private get powerBar(): PowerBar | null | undefined {
    return this.userSeat?.powerBar
  }

  private registerSocketListeners(): void {
    this.user.socket.on(SocketEvents.GAME_POWER_APPLY, this.onApplyPower)
  ...
  private cleanupSocketListeners(): void {
    this.user.socket.off(SocketEvents.GAME_POWER_APPLY, this.onApplyPower)
  }
  ...
  private handleApplyPower({
    ...
    if ("powerLevel" in powerItem) {
      powerItem = TowersPieceBlock.fromPlainObject(powerItem as TowersPieceBlockPlainObject)
      ...
      powerItem = SpecialDiamond.fromPlainObject(powerItem as SpecialDiamondPlainObject)

      switch (powerItem.powerType) {
        case "speed drop":
          this.specialSpeedDrop()
  ...
  public usePower(targetSeatNumber?: number): void {
    ...
    const powerItem: PowerBarItem | null | undefined = this.userSeat?.powerBar?.useItem()
    if (!powerItem) {
    ...
    const allActiveUsers: User[] = this.users.filter((user: User) => {
      const seat: TableSeat | undefined = this.tableSeatManager.getTableSeat(user)
      return seat && user.isPlayingInTable(this.tableId) && !seat.board?.isGameOver
    })

    const partner: User | undefined = this.users.find((user: User) => this.isPartner(user))
    const isOpponent = (user: User): boolean => user.user.id !== this.user.user.id && !this.isPartner(user)
    const opponents: User[] = allActiveUsers.filter((user: User) => isOpponent(user))

    const sendPowerToTarget = (target: User): void => {
      if (!this.userSeat) return

      if (isAttackPower && this.isPartner(target) && typeof targetSeatNumber === "undefined") {
        logger.warn(`Blocked spacebar attack to partner: ${target.user.username}`)
        return
      }

      this.targetUser = target

      this.user.io.to(this.userSeat?.tableId).emit(SocketEvents.GAME_POWER_FIRE, {
        sourceUsername: this.user.user.username,
        targetUsername: target.user.username,
        targetSeatNumber: target.getSeatNumber(this.tableId),
    ...
    if (typeof targetSeatNumber !== "undefined") {
      const targetUser: User | undefined = allActiveUsers.find((user: User) => {
        const seat: TableSeat | undefined = this.tableSeatManager.getTableSeat(user)
    ...
    // Handle spacebar press (no target specified)
      ...
      if (isAttackPower && opponents.length > 0) {
        const targetUser: User = opponents[Math.floor(Math.random() * opponents.length)]
        sendPowerToTarget(targetUser)
      } else if (isDefensePower) {
        // Defense to self
        this.handleApplyPower({
          sourceUsername: this.user.user.username,
          targetUsername: this.user.user.username,
          powerItem,
        })

        // Also send to partner, if they exist and playing
        if (typeof partner !== "undefined" && this.userSeat?.tableId) {
          const partnerSeat: TableSeat | undefined = this.tableSeatManager.getTableSeat(partner)

          if (typeof partnerSeat !== "undefined" && !partnerSeat?.board?.isGameOver) {
   ...
   */
  private isPartner(user: User): boolean {
    const seat: TableSeat | undefined = this.tableSeatManager.getTableSeat(user)
    return (
      !!this.userSeat && !!seat && seat.teamNumber === this.userSeat.teamNumber && user.user.id !== this.user.user.id
    )
  }
  ...
  private medusaPiece(): void {
    if (!this.userSeat) return
    this.userSeat.nextPieces?.addMedusaPiece()
  }
  ...
  private midasPiece(): void {
    if (!this.userSeat) return
    this.userSeat.nextPieces?.addMidasPiece()
  }
   ...
   */
  private specialSpeedDrop(): void {
    if (!this.targetUser) return

    const seatNumber: number | undefined = this.targetUser.getSeatNumber(this.tableId)
    this.user.io.to(this.tableId).emit(SocketEvents.PIECE_SPEED, {
