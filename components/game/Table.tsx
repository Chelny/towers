...
import { PlayerListWithRelations } from "db";
...
import { TablePlayerAction } from "@/interfaces/tablePlayerState";
...
export default function Table(): ReactNode {
  ...
  const { addJoinedTable, removeJoinedTable, setActiveTableId } = useGame();
  const { openModal } = useModal();
  const isJoinedTableRef = useRef<boolean>(false);
  ...
  const [table, setTable] = useState<TowersTableWithRuntime>();
  const [tablePlayer, setTablePlayer] = useState<TowersTablePlayerWithRuntime>();
  const [controlKeys, setControlKeys] = useState<TowersPlayerControlKeys | null>(null);
  const [teams, setTeams] = useState<ServerTowersTeam[]>([]);
  ...
  const { trigger: joinTable, isMutating: isJoinTableMutating } = useSWRMutation(
    `/api/games/towers/tables/${tableId}/players`,
    (url: string) => fetcher<TowersTableWithRuntime>(url, { method: "POST" }),
    {
      onSuccess(response: ApiResponse<TowersTableWithRuntime>) {
        if (response.success) {
          const table: TowersTableWithRuntime | undefined = response.data;
          if (table) {
            setTable(table);
            setTablePlayer(
              table.players?.find(
                (tablePlayer: TowersTablePlayerWithRuntime) => tablePlayer.player.id === session?.user.id,
              ),
            );

            if (!joinedTableSidebarRef.current.has(table.id)) {
              addJoinedTable({
                id: table.id,
                roomId: table.roomId,
  ...
  const { trigger: updateTableOptions, isMutating: isUpdateTableOptionsMutating } = useSWRMutation(
    `/api/games/towers/tables/${tableId}`,
    (url: string, { arg }: { arg: ChangeTableOptionsPayload }) =>
      fetcher<TowersTableWithRuntime>(url, { method: "PATCH", body: JSON.stringify(arg) }),
  );

  const { trigger: sendChatMessage, isMutating: isChatMessageMutating } = useSWRMutation(
    `/api/games/towers/tables/${tableId}/chat-messages`,
    (url: string, { arg }: { arg: { text?: string; type?: TableChatMessageType; textVariables?: JsonValue } }) =>
      fetcher<TowersTableChatMessageWithRelations>(url, { method: "POST", body: JSON.stringify(arg) }),
    {
      onSuccess(response: ApiResponse<TowersTableChatMessageWithRelations>) {
        if (response.success) {
  ...
  const { trigger: sitUser } = useSWRMutation(
    `/api/games/towers/tables/${tableId}/sit`,
    (url: string, { arg }: { arg: { seatNumber: number } }) =>
      fetcher<void>(url, { method: "PATCH", body: JSON.stringify(arg) }),
  );

  const { trigger: standUser } = useSWRMutation(`/api/games/towers/tables/${tableId}/stand`, (url: string) =>
    fetcher<void>(url, { method: "PATCH" }),
  );

  const { trigger: quitTable, isMutating: isQuitTableMutating } = useSWRMutation(
    `/api/games/towers/tables/${tableId}/players`,
    (url: string) => fetcher<void>(url, { method: "DELETE" }),
    {
      onSuccess(response: ApiResponse<void>) {
        if (response.success) {
          socketRef.current?.emit(SocketEvents.SOCKET_LEAVE, { tableId });
  ...
  const getSeatNumber = (table: TowersTableWithRuntime | undefined, playerId: string | undefined): number | null => {
    if (!table || !playerId) return null;

    const seat: TowersTableSeatWithRelations | undefined = table.seats.find(
      (seat: TowersTableSeatWithRelations) => seat.occupiedByPlayerId === playerId,
    );
    return seat?.seatNumber ?? null;
  };

  const seatNumber: number | null = useMemo(() => getSeatNumber(table, session?.user.id), [table, session?.user.id]);

  const rotatedTeams: ServerTowersTeam[] = useMemo(() => {
    if (!seatNumber) return teams; // Not seated, no rotation

    const myTeamNumber: number = Math.ceil(seatNumber / 2);

    if (myTeamNumber === 1) {
      // Full rotation for team 1, same as before
      const teamOrder: number[] = [1, 3, 2, 4];
      const myTeamIndex: number = teamOrder.indexOf(myTeamNumber);
      const rotatedOrder: number[] = [...teamOrder.slice(myTeamIndex), ...teamOrder.slice(0, myTeamIndex)];

      return rotatedOrder.map(
        (teamNumber: number) =>
          teams.find((team: ServerTowersTeam) => team.teamNumber === teamNumber) ?? { teamNumber, seats: [] },
      );
    } else {
      // Swap team 1 and the user's team, leave other teams intact
      return teams.map((team: ServerTowersTeam) => {
        if (team.teamNumber === 1) {
          return teams.find((t: ServerTowersTeam) => t.teamNumber === myTeamNumber)!;
        }

        if (team.teamNumber === myTeamNumber) {
          return teams.find((t: ServerTowersTeam) => t.teamNumber === 1)!;
        }
  ...
  const readyTeamsCount: number = useMemo(() => {
    ...
    for (const team of teams) {
      const seatedUsers: PlayerListWithRelations[] = team.seats
        .map((seat: ServerTowersSeat) => seat.occupiedByPlayer)
        .filter((player: PlayerListWithRelations | null): player is PlayerListWithRelations => !!player);

      if (seatedUsers.length === 0) continue;

      const isTeamReady: boolean = seatedUsers.every((player: PlayerListWithRelations) => {
        const tablePlayer: TowersTablePlayerWithRuntime | undefined = table?.players.find(
          (tp: TowersTablePlayerWithRuntime) => tp.playerId === player.id,
        );

        return !!tablePlayer?.isReady;
      });

      if (isTeamReady) count++;
    }
  ...
  useEffect(() => {
    if (tableId && !isJoinedTableRef.current) {
      joinTable();
      isJoinedTableRef.current = true;
    }
  }, [tableId]);

  useEffect(() => {
    if (!tablePlayer) return;
    setControlKeys(tablePlayer.player.controlKeys);
  }, [tablePlayer]);

  useEffect(() => {
    if (!table?.seats) return;
    setTeams(seatsGroupByTeam(table.seats));
  }, [table?.seats, seatNumber]);

  useEffect(() => {
    const handleFKeyMessage = async (event: KeyboardEvent): Promise<void> => {
  ...
  useEffect(() => {
    if (!socketRef.current || !roomId || !tableId) return;

    const handlePlayerJoin = ({ tablePlayer }: { tablePlayer: TowersTablePlayerWithRuntime }): void => {
      setTable((prev: TowersTableWithRuntime | undefined) => {
        if (!prev) return prev;

        const isPlayerExists: boolean = prev.players.some(
          (currentTablePlayer: TowersTablePlayerWithRuntime) => currentTablePlayer.playerId === tablePlayer.playerId,
        );
      ...
      if (tablePlayer.player.id === session?.user.id) {
        setTablePlayer(tablePlayer);
      }
    };

    const handlePlayerLeave = ({ tablePlayer }: { tablePlayer: TowersTablePlayerWithRuntime }): void => {
      setTable((prev: TowersTableWithRuntime | undefined) => {
        if (!prev) return prev;
    ...
    const handleUpdateTable = ({ table }: { table: TowersTableWithRuntime }): void => {
      setTable(table);
      setTablePlayer(
        table.players?.find((tablePlayer: TowersTablePlayerWithRuntime) => tablePlayer.player.id === session?.user.id),
      );
    };

    const handleUpdateChatMessages = ({ chatMessage }: { chatMessage: TowersTableChatMessageWithRelations }): void => {
      setTable((prev: TowersTableWithRuntime | undefined) => {
        if (!prev) return prev;
    ...
    const handleUpdateTableSeat = ({ tableSeat }: { tableSeat: TowersTableSeatWithRelations }): void => {
      setTable((prev: TowersTableWithRuntime | undefined) => {
        if (!prev) return prev;

        return {
          ...prev,
          seats: prev.seats.map((seat: TowersTableSeatWithRelations) => (seat.id === tableSeat.id ? tableSeat : seat)),
        };
      });
    };

    const handleUpdatePlayerState = ({
      playerId,
      isReady,
      isPlaying,
    }: {
      ...
      setTable((prev: TowersTableWithRuntime | undefined) => {
        ...
        return {
          ...prev,
          players: prev.players.map((tablePlayer: TowersTablePlayerWithRuntime) =>
            tablePlayer.playerId === playerId
              ? {
                  ...tablePlayer,
                  isReady,
                  isPlaying,
      ...
      setTablePlayer((prev: TowersTablePlayerWithRuntime | undefined) => {
        if (!prev || prev.playerId !== playerId) return prev;
        return { ...prev, isReady, isPlaying };
      });
    };

    const handleUpdateControlKeys = ({ controlKeys }: { controlKeys: TowersPlayerControlKeys }): void => {
    ...
    const handleGameOver = ({
      ...
      winners: TowersPlayerWithRelations[]
      isWinner: boolean
    ...
    socketRef.current?.on(SocketEvents.TABLE_MESSAGE_SENT, handleUpdateChatMessages);
    socketRef.current?.on(SocketEvents.TABLE_SEAT_UPDATED, handleUpdateTableSeat);
    socketRef.current?.on(SocketEvents.TABLE_PLAYER_STATE_UPDATED, handleUpdatePlayerState);
    socketRef.current?.on(SocketEvents.GAME_CONTROL_KEYS_UPDATED, handleUpdateControlKeys);
    ...
    return () => {
      ...
      socketRef.current?.off(SocketEvents.TABLE_MESSAGE_SENT, handleUpdateChatMessages);
      socketRef.current?.off(SocketEvents.TABLE_SEAT_UPDATED, handleUpdateTableSeat);
      socketRef.current?.off(SocketEvents.TABLE_PLAYER_STATE_UPDATED, handleUpdatePlayerState);
      socketRef.current?.off(SocketEvents.GAME_CONTROL_KEYS_UPDATED, handleUpdateControlKeys);
  ...
  }, [isConnected, roomId, tableId]);

  const handleStart = async (): Promise<void> => {
    socketRef.current?.emit(SocketEvents.TABLE_PLAYER_STATE_UPDATE, { tableId, action: TablePlayerAction.READY });
  };

  const handleSit = async (seatNumber: number): Promise<void> => {
    await sitUser({ seatNumber });
  };
  ...
  const handleQuitTable = async (): Promise<void> => {
    await quitTable();
  };

  const handleSendMessage = async (event: React.KeyboardEvent<HTMLInputElement>): Promise<void> => {
      ...
      >
        ...
        >
          <div className="flex flex-col gap-2">
            <Button
              className="w-full"
              disabled={!isConnected || !seatNumber || tablePlayer?.isReady || gameState === GameState.PLAYING}
              onClick={handleStart}
            >
              <Trans>Start</Trans>
            </Button>
            ...
            <Select
              ...
              disabled={!isConnected || session?.user.id !== table?.hostPlayer?.id || isUpdateTableOptionsMutating}
              isNoBottomSpace
              onChange={() => {
                handleOptionChange();
              }}
            ...
            <Button
              className="w-full"
              disabled={!isConnected || session?.user.id !== table?.hostPlayer?.id}
            ...
            <Checkbox
              ...
              disabled={
                !isConnected ||
                session?.user.id !== table?.hostPlayer?.id ||
                gameState === GameState.COUNTDOWN ||
                gameState === GameState.PLAYING ||
                isUpdateTableOptionsMutating
              }
              isNoBottomSpace
          ...
          <div className="flex gap-1">
            ...
            <Button className="w-full" disabled={!isConnected || isQuitTableMutating} onClick={handleQuitTable}>
              <Trans>Quit</Trans>
            </Button>
          </div>
        </div>
            ...
            >
              ...
              >
                <div className="relative grid [grid-template-areas:'timer_team1_team3''timer_team1_team3''team2_team1_team4''team2_hint_team4'] grid-cols-(--grid-cols-table-team) w-fit p-2 mx-auto">
                  ...
                  {/* Game */}
                  <>
                    {isJoinTableMutating || !teams.length ? (
                      <>
                        {seatsGroupByTeam(
                          Array.from({ length: NUM_TABLE_SEATS }, (_, index: number) => ({
                            id: `skeleton-${index}`,
                            seatNumber: index + 1,
                            teamNumber: Math.floor(index / 2) + 1,
                            tableId: "skeleton-table-id",
                            table: {} as TowersTable,
                            occupiedByPlayerId: null,
                            occupiedByPlayer: null,
                            createdAt: new Date(),
                            updatedAt: new Date(),
                          })),
                        ).map((team: ServerTowersTeam, index: number) => (
                          <div
                            key={index}
                          ...
                          >
                            <div className={index === 0 ? "contents" : "flex flex-row justify-center items-center"}>
                              {team.seats.map((seat: ServerTowersSeat) => (
                                <PlayerBoardSkeleton
                                  key={seat.seatNumber}
                      ...
                      <>
                        {rotatedTeams.map((team: ServerTowersTeam, index: number) => {
                          const isUserSeatedAnywhere: boolean = teams.some((team: ServerTowersTeam) =>
                            team.seats.some((seat: ServerTowersSeat) => seat.occupiedByPlayer?.id === session?.user.id),
                          );
                          ...
                          return (
                            ...
                            >
                              <div className={index === 0 ? "contents" : "flex flex-row justify-center items-center"}>
                                {team.seats.map((seat: ServerTowersSeat) => {
                                  const seatTablePlayer: TowersTablePlayerWithRuntime | undefined =
                                    table?.players?.find(
                                      (tablePlayer: TowersTablePlayerWithRuntime) =>
                                        tablePlayer.playerId === seat.occupiedByPlayer?.id,
                                    );

                                  const isSitAccessGranted: boolean =
                                    table?.hostPlayerId === tablePlayer?.playerId ||
                                    table?.tableType === TableType.PUBLIC ||
                                    ((table?.tableType === TableType.PROTECTED ||
                                      table?.tableType === TableType.PRIVATE) &&
                                      !!tablePlayer?.player?.receivedTableInvitations.some(
                                  ...
                                  return (
                                    <PlayerBoard
                                      ...
                                      isTablePlayerReady={!!seatTablePlayer?.isReady}
                                      isTablePlayerPlaying={!!seatTablePlayer?.isPlaying}
                                      isRatingsVisible={table?.room?.level !== RoomLevel.SOCIAL}
                                      onSit={handleSit}
                                      onStand={handleStand}
                                      onStart={handleStart}
